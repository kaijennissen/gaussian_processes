...
class GPR:
    ...
    def sample_posterior(self, X_train, y_train, X_test, n_samples=1):
        ...
        return (
            self._sample_multivariate_gaussian(y_mean, y_cov, n_samples),
            y_mean,
            y_cov,
        )
    ...
    def _cholesky_factorise(self, y_cov):
        try:
            ...
            e.args = (
                ...
                " a larger value for epsilon. " % self.kernel,
            ) + e.args
            raise
        return L
...
class RationalQuadratic:
    ...
    def __call__(self, X1, X2=None):
        if X2 is None:
            # K(X1, X1) is symmetric so avoid redundant computation using pdist.
            dists = pdist(
                X1 / self.theta[0] * np.sqrt(self.theta[1]), metric="sqeuclidean"
            )
            ...
            dists = cdist(
                X1 / self.theta[0] * np.sqrt(self.theta[1]),
                X2 / self.theta[0] * np.sqrt(self.theta[1]),
                metric="sqeuclidean",
            )
            K = (1 + dists) ** (-self.theta[1])
        return K
    ...
    def __call__(self, X1, X2=None):
        if X2 is None:
            ...
            dists = pdist(
                X1,
                lambda xi, xj: np.dot(
                    np.sin(self.theta[0] * np.pi * (xi - xj)).T,
